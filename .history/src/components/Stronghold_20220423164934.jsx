            ...
            if (_pid == 1) {
              ...
              await Promise.all(
                tokenIdsArray.map(async (_tindex) => {
                  ...
                  if (ownerAddress.toLowerCase() !== '0x000000000000000000000000000000000000dEaD'.toLowerCase() && ownerAddress.toLowerCase() === address.toLowerCase()) {
                    const tokenUri = await poolContract.methods
                      .tokenURI(_tindex)
                      .call();
                    const response = await fetch(
              ...
              await Promise.all(
                tokenIdsArray.map(async (_tindex) => {
                  const ownerAddress = await poolContract.methods
                    .ownerOf(_tindex)
                    .call().then((result) => {
                      if (result.toLowerCase() !== '0x000000000000000000000000000000000000dEaD'.toLowerCase() && ownerAddress.toLowerCase() === address.toLowerCase()) {
                        console.log(_tindex);
                        poolContract.methods
                          .tokenURI(_tindex)
                          .call().then((uri) => {
                            const response = fetch(
                              uri.replace('https://gateway.pinata.cloud/ipfs/', 'http://infura-ipfs.io/ipfs/')
                            );
                            let jsonifyResp = response.json();
                            jsonifyResp.tokenId = _tindex;
                            ownedNftsInfo.push(jsonifyResp);
                          })
                      }
