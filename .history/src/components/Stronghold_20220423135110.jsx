  ...
  const testPoolInfo = async () => {
    ...
    await Promise.all(
      idsArray.map(async (_pid) => {
        ...
        const firstTokenUri = await poolContract.methods.tokenURI(1).call();
        const responseFT = await fetch(
          firstTokenUri.replace('ipfs://', 'http://cloudflare-ipfs.com/ipfs/')
        );
          ...
          await Promise.all(
            totalAmountArray.map(async (_tid) => {
              ...
              const response = await fetch(
                tokenUri.replace('ipfs://', 'http://cloudflare-ipfs.com/ipfs/')
              );
