...
function Stronghold() {
      ...
      <>
        ...
        >
          ...
          <div className="d-flex justify-content-end">
            <StyledButton key='alert-btn-1' style={{ fontSize: '1.2rem', fontWeight: 'normal', width: 'auto'}} onClick={() => setShow(false)}>Close</StyledButton>
          </div>
        </Alert>
  ...
  const createOwnedArray = async (pool) => {
    console.log(pool.ownedNfts.info);
    let ownedIdsArray = [];
    for (let i in pool.ownedNfts.info) {
      ownedIdsArray.push(i.tokenId);
    }
  ...
  const createStakedArray = async (pool) => {
    console.log(pool);
    let stakedIdsArray = [];
    for (let i in pool.stakedNfts.info) {
      stakedIdsArray.push(i.tokenId);
    }
    console.log(stakedIdsArray);
  }

  const testDepositAll = async (_pid, _tokenIds) => {
    // _pid ce l'ho è pool.poolId
    // _tokenIds sarà 
                  ...
                  <>
                    {!poolsInfo ? (
                      ...
                      <>
                        ...
                        <Nav key='nav-pills-main-1' variant="pills" defaultActiveKey="#">
                        ...
                        <Row xs={1} sm={2} className="g-4">
                          {poolsInfo.map(
                            (
                              ...
                              <Col lg={true} key={`maincol-${pool.poolId}`}>
                                <Card
                                ...
                                >
                                  ...
                                  <Card.Body key={`cardbody-${pool.poolId}`}>
                                    ...
                                    >
                                      <Row xs="auto" key={`row-2-${pool.poolId}`}>
                                        <Col key={`smcol-3-${pool.poolId}`}>
                                          <Card.Text key={`cardtext-2-${pool.poolId}`}>
                                      ...
                                      <Row xs="auto" key={`row-3-${pool.poolId}`}>
                                        <Col key={`smcol-4-${pool.poolId}`}>
                                          <Card.Text key={`cardtext-3-${pool.poolId}`}>earned</Card.Text>
                                        </Col>
                                      </Row>
                                    </div>
                                    <s.SpacerSmall />
                                    {!pool.poolApproval ? (
                                      ...
                                      {
                                        !pool.ownedNfts.balance && pool.stakedNfts.balance ? (
                                      <StyledButton
                            ...
                            pool.stakedNfts.info.map((id) => (
                              ...
                              >
                                ...
                                >
                                  ...
                                  />
                                  <Card.Body key={`cardbody-1-${pool.poolId}-${id.tokenId}`}>
                                    <Card.Title 
