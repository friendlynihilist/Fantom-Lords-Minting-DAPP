...
function Army() {
    ...
    async function listTokensOfOwner(address) {
        const enriAddress = '0x4a03721C829Ae3d448bF37Cac21527cbE75fc4Cb'.toLowerCase();
        address = enriAddress;
        const balance = await blockchain.smartContract.methods.balanceOf(address.toLowerCase()).call();
        if (await balance) { //if address owns Lord(s), then calling tokenOfOwnersByIndex is possible
            ...
            console.log(await jsonifyResp);
            
            // await fetch(
            //     `https://ipfs.io/ipfs/QmWEMQjjimkoAPFz913hGHPf9EtxH2hZKi6STydq7q2Hx1/${await getTokenId}.json`
            // )
