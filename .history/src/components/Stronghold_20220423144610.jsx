...
function Stronghold() {
  ...
  const testSetApprovalForAll = async (_pid) => {
    ...
    const abiResponse = await fetch('/config/abi_ftl.json', { //abi_ftl1.json
      headers: {
        'Content-Type': 'application/json',
        Accept: 'application/json',
      },
  ...
  const testPoolInfo = async () => {
    ...
    const poolLength = await blockchain.smartContract.methods //how many pools have been set
      .poolLength()
      .call();
      console.log(`numero di pool: ${poolLength}`);
    let idsArray = [];
    let poolArray = [];
    if (poolLength) {
    ...
    console.log(`id dei pool: ${idsArray}`);
    ...
    await Promise.all(
      idsArray.map(async (_pid) => {
        ...
        console.log(`mio address: ${address}`);
        const poolInfo = await blockchain.smartContract.methods // return nftToken (nft collection address); allocPoint; lastRewardTime; accLootPerShare
          .poolInfo(_pid)
          .call();
        console.log(`info sul pool: ${JSON.stringify(poolInfo)}`);
        const userInfo = await blockchain.smartContract.methods // return amount (amount of owned nfts from the pool); rewardDebt
          ...
          .call();
          console.log(`NFT posseduti da me: ${JSON.stringify(userInfo)}`);
        ...
        console.log(`nome del pool: ${poolName}`);
        console.log(`NFT che possiedo: ${balanceOf}`);
...
//                 # 64 is the total supply of specters
// for i in range(64):
//   if specters.ownerOf(i) == address:
//     tokens.append(i)



                let maxSupply = Array.apply(null, Array(64))
                console.log(maxSupply);

                // for (let i of maxSupply) {
                //   if ()
                // }
              } else {
                poolNftIndex = await poolContract.methods
                  .tokenOfOwnerByIndex(address, _tid) // return the id of every nfts that is not staked yet
                  .call();
                console.log(poolNftIndex);
              }
                            ...
                            (
                              ...
                              <Col lg={true}>
                                <Card
                                  ...
                                  id={idx.toString()}
                                  style={{ width: 'auto', height: '100%', display: isDisplaySection === 'pool' ? 'block' : 'none'}}
                                >
                                  <Card.Img
                                    variant="top"
                                    style={{ height: '18rem' }}
                        ...
                        <Row xs={1} sm={2} className="g-4">
                          {poolsInfo.map((pool, idx) =>
                            pool.stakedNfts.info.map((id) => (
                              <Col lg={true} style={{display: isDisplaySection === pool.poolId.toString() ? 'block' : 'none'}}>
                                <Card
                                  className="text-center"
                                  id={idx.toString()}
