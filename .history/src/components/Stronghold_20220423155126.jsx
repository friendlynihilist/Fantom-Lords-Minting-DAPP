  ...
  const testPoolInfo = async () => {
    ...
    await Promise.all(
      idsArray.map(async (_pid) => {
        ...
        console.log(`ARRIVA FIN QUI ${firstTokenId}`);
        ...
        let URIprefix = jsonifyRespFT.image.startsWith('https://ipfs.io/ipfs/') ? 'https://ipfs.io/ipfs/' : 'ipfs://';
            ...
            if (_pid == 1) {
              ...
              await Promise.all(
                tokenIdsArray.map(async (_tindex) => {
                  ...
                  .ownerOf(_tindex)
                  .call();
                  console.log(`ECCO QUA: ${poolNftIndex} E mio ${address}`);
                  if (poolNftIndex === address) {
                    console.log(`ECCO QUA: ${_tindex}`);
                  }
                })
              )
            } else if (_pid == 2) {
              for (let index = 0; index < 82; index++) {
                let isOwner = await poolContract.methods.ownerOf(index);
                if (isOwner === address) {
                  console.log(`ECCO QUA: ${index}`);
                }
              }
            }
          }
