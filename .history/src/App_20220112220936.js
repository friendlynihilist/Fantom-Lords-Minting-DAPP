...
function App() {
  ...
  function getMessage(id) {
    return new Promise(function (resolve, reject) {
      resolve(blockchain.smartContract.methods.ownerOf(id).call());
    });
  }
  ...
  async function main(address) {
    for (let i = 1; i <= 3333; i++) {
      ...
      if (message.toLowerCase() === address.toLowerCase()) {
        console.log(ownedLordsArray);
        ownedLordsArray.push(i);
      }
    }
  }
  ...
  const testPromise = () => {
    ...
    for (let i = 0; i < todos.length; i++) {
      promiseArray.push(
        fetch('https://jsonplaceholder.typicode.com/todos/' + todos[i])
      );
    }
  ...
  const getOwnerOf = (owner) => {
    ...
    for (let i = 1; i < 3333; i++) {
      ...
      ownerAddress.then(function (result) {
        // console.log(result);
  ...
  const fetchRarity = (id) => {
    ...
    if (typeof id != null) {
      ...
      fetch(
        `https://ipfs.io/ipfs/QmWEMQjjimkoAPFz913hGHPf9EtxH2hZKi6STydq7q2Hx1/${id}.json`
      )
        .then((response) => response.json())
        .then(
          (data) => (document.getElementById('hero-name').innerHTML = data.name)
        );
    } else {
            ...
            {Number(data.totalSupply) >= CONFIG.MAX_SUPPLY ? (
              <>
                ...
                <StyledButton
                  ...
                  onClick={(e) => {
                    main('0x4a03721C829Ae3d448bF37Cac21527cbE75fc4Cb'); //blockchain.account
                  }}
        ...
        >
          ...
          >
            ...
            <span style={{ fontStyle: 'italic' }}>
              And you shall see your Lord's fate...
            </span>
          </s.TextDescription>
          <s.SpacerMedium />
          ...
          {printID && Number(fetchID) <= data.totalSupply ? (
            <StyledViewer
              src={viewImage(fetchID)}
              style={{ border: '5px solid black' }}
            ></StyledViewer>
          ) : (
            <StyledViewer src={crystalBall}></StyledViewer>
          )}
          {/* {
          ...
          {printID && fetchID && Number(fetchID) <= data.totalSupply ? (
            <DivTitle style={{ paddingTop: 15 }} id="hero-name">
              Fantom Lord # {fetchID}
            </DivTitle>
          ) : (
            ''
          ...
          {data.totalSupply != 0 ? (
            ...
            >
              You must attune before venturing forth...
            </span>
          )}
          {data.totalSupply != 0 ? (
            <StyledButton onClick={() => setPrint(true)}>Ponder</StyledButton>
          ) : (
            ''
          )}
          ...
          >
            {/* {actualDate.getTime() < mintDate.getTime() ? 'NOT READY' : 'READY'} */}
            <span style={{ fontWeight: 'bold' }}>
              Or browse Fantom Lords' rarity and rankings at{' '}
              <a
                href="https://nftkey.app/collections/fantomlords/"
                target={'_blank'}
              >
